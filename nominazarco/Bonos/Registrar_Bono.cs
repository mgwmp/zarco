...
{
    public partial class Registrar_Bono : Form
    {
        ...
        private void btn_Aceptar_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("INSERT INTO bono (fecha_creacion, cantidad, id_empleado)" +
                                                    " VALUES('" + dtp_FechaBono.Value.ToString(isoDateTimeFormat) + "','" + txt_CantidadBono.Text + "','" + txt_ID.Text + "');", cn);
                    cmd.CommandType = CommandType.Text;
                    cmd.ExecuteScalar();
                    MessageBox.Show("Bono registrado con exito.");
                    this.Close();
                }
                catch (Exception df)
                {
                    MessageBox.Show("Ocurrio un error al intentar registrar el bono.\n Error : \n" + df.ToString(), "Mensaje de error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                }
            }
        }
